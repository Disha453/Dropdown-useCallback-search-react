...
function App() {
  ...
  const [data, setData] = useState(JSON.parse(localStorage.getItem("z")) || [])
  ...
  const submit = () => {
    ...
    localStorage.setItem("z", JSON.stringify([...data, student]));

  }

  const hendelsearch = useCallback((idx) => {
    if (selected === "w") {
      return (data.filter((item) => item?.name?.toLocaleLowerCase().includes(idx?.toLocaleLowerCase())));
    }
  ...
  return (
    <>
      ...
      <div className="h-[950px]  bg-cover p-2.5  flex  flex-col items-center  justify-center " style={{ backgroundImage: "url(https://i.pinimg.com/736x/b0/e8/28/b0e828d20d13d3c2b0e11250a9093288.jpg)" }}>
        ...
        <div>
          <select onChange={(e) => setSelected(e.target.value)} className="bg-transparent h-8 w-36 rounded-xl  border-blue-500  mr-[50px]  text-[15px]  mt-[15px]">
